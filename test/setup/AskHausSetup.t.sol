...
contract AskHausSetupLive is BaalSetupLive, Accounts {
    ...
    uint256 SNAPSHOT_BLOCK = DELEGATE_TIMESTAMP + 15;
    uint256 VOTE_BLOCK = DELEGATE_TIMESTAMP + 20;
    ...
    function __setupAskHausPoll() internal {
    ...
    function _fastFactorySetup() internal {
        ...
        BaalPointsV0 _baalPointsImpl = new BaalPointsV0();
        BaalGateV0 _baalChoicesImpl = new BaalGateV0();
        Prepop _prepopChoicesImpl = new Prepop();
        TimedVotes _timedVotesModuleImpl = new TimedVotes();
        EmptyExecution _executionModuleImpl = new EmptyExecution();
        ...
        factory().setModuleTemplate(_timedVotesModuleImpl.MODULE_NAME(), address(_timedVotesModule), _mockMetadata);
        factory().setModuleTemplate(_executionModuleImpl.MODULE_NAME(), address(_executionModule), _mockMetadata);
        ...
        assertEq(factory().moduleTemplates(_timedVotesModuleImpl.MODULE_NAME()), address(_timedVotesModule));
        assertEq(factory().moduleTemplates(_executionModuleImpl.MODULE_NAME()), address(_executionModule));
    }
